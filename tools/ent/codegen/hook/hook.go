...
import (
	...
	"github.com/alexgtn/go-linkshort/tools/ent/codegen"
)

// The NordPoolDayAheadFunc type is an adapter to allow the use of ordinary
// function as NordPoolDayAhead mutator.
type NordPoolDayAheadFunc func(context.Context, *codegen.NordPoolDayAheadMutation) (codegen.Value, error)

// Mutate calls f(ctx, m).
func (f NordPoolDayAheadFunc) Mutate(ctx context.Context, m codegen.Mutation) (codegen.Value, error) {
	mv, ok := m.(*codegen.NordPoolDayAheadMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *codegen.NordPoolDayAheadMutation", m)
	}
	return f(ctx, mv)
}
